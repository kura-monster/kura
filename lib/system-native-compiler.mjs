...
import { analyzeNativeSafety, assertNativeSafety } from './system-native-safety.mjs';
...
export class NativeCompiler {
  ...
  validate() {
    ...
    this.safetyReport = analyzeNativeSafety(this.program, this, {
      mode: this.options.safetyMode,
      denyUndocumentedUnsafe: this.options.denyUndocumentedUnsafe,
    });
    assertNativeSafety(this.safetyReport, this.context);
  }
...
export function analyzeNativeSystemSafety(source, options = {}) {
  const compiler = new NativeCompiler(parseNativeSystemSource(source, options), {
    ...options,
    safetyMode: options.safetyMode ?? 'audit',
  });
  compiler.validate();
