...
export function createHardwareKernelSource(options = {}) {
  ...
  return `${prefix}const PCI_CONFIG_ADDRESS_PORT: u16 = 0xCF8
...
export async function buildHardwareKernel(source = null, options = {}) {
  ...
  if (!options.dryRun) {
    ...
    await writeFile(plan.hardwareManifest, JSON.stringify(createHardwareManifest(options), null, 2) + '\n', 'utf8');
  }
...
export async function hardwareSmokeTest() {
  ...
  return { ok: completion.success, commandId: submitted.commandId, xhciType: ring.hardwarePop().type, key: keyboard.keys[0], txQueued: net.stats().queuedTx, framebuffer: framebuffer.checksum(), fingerprint: hardwareFingerprint() };
}
