...
export function createHardwareQemuSmokeSource(options = {}) {
  const completeSource = createCompleteHardwareKernelSource({ ...options, smoke: true });
  const checkpoint = '    memory_init_from_multiboot(boot_info)\n    heap_init()';
  if (!completeSource.includes(checkpoint)) throw new Error('Generated smoke kernel has no Multiboot memory checkpoint.');
  return completeSource.replace(checkpoint, `    memory_init_from_multiboot(boot_info)
    let smoke_acpi_root: usize = acpi_root_table()
    let smoke_mcfg: usize = acpi_find_table(ACPI_SIGNATURE_MCFG)
    if smoke_acpi_root == 0 || smoke_mcfg == 0 {
        serial_write_byte(0x45)
        io.out32(0xF4, SMOKE_EXIT_CODE + 1)
        cpu.halt()
    }
...
export async function runHardwareQemuSmoke(options = {}) {
  ...
  const smokeSource = createHardwareQemuSmokeSource(options);
  ...
  return { build, iso, run, outputs: isoPlan, checkpoint: { stage: 'acpi-root-and-mcfg-discovered', successSerialByte: 0x41, failureSerialByte: 0x45, deviceMmioTouched: false } };
}
