...
class Parser {
  ...
  statement() {
    if (this.match('unsafe')) {
      ...
      let reason = null;
      if (this.match('(')) {
        if (this.current().type !== 'string') fail('unsafe(reason) requires a quoted safety justification.', this.current(), this, 'KR-NATIVE-PARSE-0011');
        reason = decodeString(this.next(), this);
        this.expect(')');
      }
      return { kind: 'UnsafeBlock', body: this.block(), reason, token };
    }
