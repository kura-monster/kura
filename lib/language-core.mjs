...
const KEYWORDS = new Set([
  ...
  'pub', 'export', 'self', 'Self', 'async', 'await', 'while', 'break', 'continue', 'null',
]);
...
class Parser {
  ...
  statement() {
    ...
    if (this.match('while')) {
      ...
      return { kind: 'WhileStatement', test, body, token };
    }
    if (this.match('break')) {
      const token = this.tokens[this.index - 1];
      this.match(';');
    ...
    if (this.match('continue')) {
      const token = this.tokens[this.index - 1];
      this.match(';');
      return { kind: 'ContinueStatement', token };
    }
    const expression = this.expression();
    if (['=', '+=', '-=', '*=', '/='].includes(this.current().value)) {
      const operator = this.next();
      if (!['Identifier', 'MemberExpression', 'IndexExpression'].includes(expression.kind)) fail('Invalid assignment target.', expression.token, this, 'KR-LANG-PARSE-0008');
      const value = this.expression();
      this.match(';');
      return { kind: 'AssignmentStatement', target: expression, op: operator.value, value, token: operator };
    }
    this.match(';');
  ...
  prefix() {
    ...
    if (current.type === 'number') node = { kind: 'NumberLiteral', value: current.value, token: current };
    ...
    else if (current.value === 'null') node = { kind: 'NullLiteral', value: null, token: current };
      ...
      this.expect(']');
      node = { kind: 'ArrayLiteral', items, token: current };
    }
  ...
  postfix(node) {
    while (true) {
      ...
      if (this.match('[')) {
        ...
        this.expect(']');
        node = { kind: 'IndexExpression', object: node, index, token: node.token };
        continue;
      }
...
function emitExpression(expression, state = {}) {
  ...
  switch (expression.kind) {
    ...
    case 'ArrayLiteral': return `[${expression.items.map(item => emitExpression(item, state)).join(', ')}]`;
    case 'Identifier': return escapeIdentifier(expression.name);
    case 'MemberExpression': return emitPath(expression);
    case 'IndexExpression': return `${emitExpression(expression.object, state)}[${emitExpression(expression.index, state)}]`;
...
function emitStatement(statement, depth = 0, state = {}) {
  ...
  switch (statement.kind) {
    ...
    case 'WhileStatement': return `${prefix}while (${emitExpression(statement.test, state)}) {\n${statement.body.body.map(item => emitStatement(item, depth + 1, state)).join('\n')}\n${prefix}}`;
    case 'AssignmentStatement': return `${prefix}${emitExpression(statement.target, state)} ${statement.op} ${emitExpression(statement.value, state)};`;
