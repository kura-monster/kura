...
const KEYWORDS = new Set([
  ...
  'export', 'async', 'await', 'trait', 'impl', 'where', 'pure', 'kernel', 'comptime', 'test',
]);

const INTERNAL_PREFIX = '__kr_';
const FORBIDDEN_BINDINGS = new Set(['__proto__', 'prototype', 'constructor', 'eval', 'Function', 'Kura']);
...
class Parser {
  ...
  declaration() {
    ...
    if (this.match('test')) return this.testDeclaration(async);
  ...
  testDeclaration(async = false) {
    const token = this.tokens[this.index - 1];
    const nameToken = this.next();
    let name;
    if (nameToken.type === 'string') {
      name = decodeStringLiteral(nameToken.value, nameToken, this);
    } else if (nameToken.type === 'identifier') {
      name = nameToken.value;
    } else {
      throw compileError('Expected a quoted test name or identifier.', nameToken, this, {
        code: 'KR-PARSE-1250',
        title: 'Kura needs a test name',
        hint: 'Example: test "adds two numbers" { assert_eq(1 + 1, 2); }',
      });
    }
    if (!name.trim() || name.length > 200) {
      throw compileError('Test names must contain between 1 and 200 characters.', nameToken, this, {
        code: 'KR-PARSE-1251', title: 'Invalid test name', hint: 'Use a short, descriptive test name.',
      });
    }
    const body = this.block();
    return { kind: 'Test', name, body, async, line: token.line, column: token.column };
  }
...
function validateProgramSecurity(ast, source, options) {
  ...
  const visit = node => {
    if (node.kind === 'Import') {
      node.source = normalizeImportSpecifier(node.source, mode, file, node, source, options);
    }
    ...
    if (mode === 'strict' && node.kind === 'Identifier' && node.name === 'Kura' && !options.allowAi) {
      throw new KuraCompileError('Strict security mode blocks Kura.ai network access.', file, node.line ?? 1, node.column ?? 1, {
        code: 'KR-SEC-1212',
        title: 'AI network capability blocked',
        hint: 'Review the endpoint and rerun with --secure --allow-ai. Pass the API key with --allow-env KURA_AI_API_KEY.',
        source,
        length: 4,
      });
    }
...
function normalizeImportSpecifier(specifier, mode, file, node, source, options = {}) {
  ...
  if (value.startsWith('std:')) {
    const moduleName = value.slice(4);
    if (!/^[a-z][a-z0-9-]{0,63}$/.test(moduleName)) {
      throw new KuraCompileError(`The standard-library module '${moduleName}' is not valid.`, file, node.line ?? 1, node.column ?? 1, {
        code: 'KR-PARSE-1210', title: 'Invalid standard-library import', hint: 'Example: import { readText } from std:"fs";', source,
      });
    }
    const sensitive = new Set(['fs', 'http', 'env', 'process', 'ai']);
    if (mode === 'strict' && sensitive.has(moduleName)) {
      throw new KuraCompileError(`Strict security mode blocks std:${moduleName}.`, file, node.line ?? 1, node.column ?? 1, {
        code: 'KR-SEC-1210', title: 'Sensitive standard-library capability blocked', hint: 'Use a capability-free std module, or run reviewed code without --secure.', source,
      });
    }
    if (!options.stdlibRoot) {
      throw new KuraCompileError('The compiler was not given a standard-library location.', file, node.line ?? 1, node.column ?? 1, {
        code: 'KR-STDLIB-0001', title: 'Standard library is unavailable', hint: 'Run this code through the kr CLI or pass stdlibRoot to compile().', source,
      });
    }
    const resolved = path.resolve(options.stdlibRoot, `${moduleName}.mjs`);
    const relative = path.relative(path.resolve(options.stdlibRoot), resolved);
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
      throw new KuraCompileError('Standard-library import escaped its root.', file, node.line ?? 1, node.column ?? 1, { code: 'KR-SEC-1211', title: 'Unsafe standard-library path', source });
    }
    return pathToFileURL(resolved).href;
  }
  if (value.startsWith('pkg:')) value = value.slice(4);
  ...
  if (mode === 'strict' && !value.startsWith('./') && !value.startsWith('../') && !value.startsWith('node:')) {
...
function optimizeStatements(body) {
  ...
  for (const node of body) {
    if (node.kind === 'Function' || node.kind === 'Test') output.push({ ...node, body: optimizeStatements(node.body) });
...
function builtin(name) {
  return ({ println: 'console.log', print: 'process.stdout.write', len: '__kr_len', str: 'String', int: 'Number', float: 'Number', range: '__kr_range', panic: '__kr_panic', assert: '__kr_assert', assert_eq: '__kr_assert_eq', assert_ne: '__kr_assert_ne', assert_deep_eq: '__kr_assert_deep_eq', fail: '__kr_fail' })[name] ?? name;
}
...
function emitStatement(node, depth = 0) {
  ...
  switch (node.kind) {
    case 'Function':
      return `${prefix}${node.exported ? 'export ' : ''}${node.async ? 'async ' : ''}function ${node.name}(${node.params.map(item => item.name).join(', ')}) {\n${node.body.map(item => emitStatement(item, depth + 1)).join('\n')}\n${prefix}}`;
    case 'Test':
      return `${prefix}${node.async ? 'async ' : ''}function ${node.internalName}() {
${node.body.map(item => emitStatement(item, depth + 1)).join('\n')}
${prefix}}`;
    case 'Variable':
    ...
    case 'If':
      return `${prefix}if (${emitExpression(node.test)}) {\n${node.consequent.map(item => emitStatement(item, depth + 1)).join('\n')}\n${prefix}}${node.alternate ? ` else {\n${node.alternate.map(item => emitStatement(item, depth + 1)).join('\n')}\n${prefix}}` : ''}`;
    case 'While':
    ...
    case 'Struct':
      return `${prefix}${node.exported ? 'export ' : ''}class ${node.name} { constructor(${node.fields.map(field => field.name).join(', ')}) { ${node.fields.map(field => `this.${field.name}=${field.name};`).join(' ')} } }`;
    case 'Enum':
      return `${prefix}${node.exported ? 'export ' : ''}const ${node.name}=Object.freeze({${node.variants.map(variant => `${variant.name}:${variant.fields.length ? `(...values)=>Object.freeze({tag:${JSON.stringify(variant.name)},values:Object.freeze(values)})` : `Object.freeze({tag:${JSON.stringify(variant.name)}})`}`).join(',')}});`;
    case 'Import':
...
function walkExpression(node, visit) {
  ...
  visit(node);
  switch (node.kind) {
    case 'Array': node.items.forEach(item => walkExpression(item, visit)); break;
    case 'Unary':
    ...
    case 'Call': walkExpression(node.callee, visit); node.args.forEach(item => walkExpression(item, visit)); break;
...
function walkStatements(body, visit) {
  for (const node of body) {
    visit(node);
    if (node.kind === 'Function' || node.kind === 'Test' || node.kind === 'While' || node.kind === 'For') walkStatements(node.body, visit);
    if (node.kind === 'If') {
      walkStatements(node.consequent, visit);
      if (node.alternate) walkStatements(node.alternate, visit);
    }
    for (const key of ['init', 'value', 'expression', 'test', 'iterable']) walkExpression(node[key], visit);
  }
}

function usesIdentifier(ast, name) {
  let found = false;
  walkStatements(ast.body, node => {
    if (node.kind === 'Identifier' && node.name === name) found = true;
  });
  return found;
}
...
function buildPrelude(body) {
  ...
  if (body.includes('__kr_assert')) lines.push('const __kr_assert = (condition, message = "Expected condition to be true") => { if (!condition) throw new Error(String(message)); };');
  if (body.includes('__kr_assert_eq')) lines.push('const __kr_assert_eq = (actual, expected, message) => { if (!Object.is(actual, expected)) throw new Error(message ?? `Expected ${JSON.stringify(actual)} to equal ${JSON.stringify(expected)}`); };');
  if (body.includes('__kr_assert_ne')) lines.push('const __kr_assert_ne = (actual, expected, message) => { if (Object.is(actual, expected)) throw new Error(message ?? `Expected ${JSON.stringify(actual)} not to equal ${JSON.stringify(expected)}`); };');
  if (body.includes('__kr_assert_deep_eq')) lines.push('const __kr_assert_deep_eq = (actual, expected, message) => { if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error(message ?? "Values are not deeply equal"); };');
  if (body.includes('__kr_fail')) lines.push('const __kr_fail = (message = "Test failed") => { throw new Error(String(message)); };');
  return lines.join('\n');
}
...
export function compile(source, options = {}) {
  ...
  let testIndex = 0;
  for (const item of ast.body) if (item.kind === 'Test') item.internalName = `__kr_test_${testIndex++}`;
  const body = ast.body.map(item => emitStatement(item, 0)).join(options.compact ? '\n' : '\n\n');
  const prelude = buildPrelude(body);
  const usesKuraAi = usesIdentifier(ast, 'Kura');
  let nativeImports = '';
  if (usesKuraAi) {
    if (!options.aiRuntime) {
      throw new KuraCompileError('The compiler was not given the Kura.ai runtime location.', options.file ?? '<input>', 1, 1, {
        code: 'KR-AI-0007', title: 'Kura.ai runtime is unavailable', hint: 'Run this program through the kr CLI or pass aiRuntime to compile().', source,
      });
    }
    nativeImports = `import { Kura } from ${JSON.stringify(pathToFileURL(path.resolve(options.aiRuntime)).href)};`;
  }
  const mainFunctions = ast.body.filter(item => item.kind === 'Function' && item.name === 'main');
  ...
  const benchmarkFunction = ast.body.find(item => item.kind === 'Function' && item.kernel);
  const exports = [];
  ...
  const testDeclarations = ast.body.filter(item => item.kind === 'Test');
  const testExpose = options.exposeTests ? `\nexport const __kr_tests = [${testDeclarations.map(item => `{name:${JSON.stringify(item.name)},fn:${item.internalName},line:${item.line ?? 1},column:${item.column ?? 1}}`).join(',')}];` : '';
  const expose = (exports.length ? `\nexport { ${exports.join(', ')} };` : '') + testExpose;
  ...
  const sections = [banner.trimEnd(), nativeImports, prelude, body + expose + epilogue].filter(Boolean);
  return {
    ast,
...
export function format(source, options = {}) {
  ...
  return ast.body.map(item => formatStatement(item, 0)).join('\n\n') + '\n';
}
...
${prefix}}`;
    ...
    case 'If': return `${prefix}if (${emitExpression(node.test)}) {\n${node.consequent.map(item => formatStatement(item, depth + 1)).join('\n')}\n${prefix}}${node.alternate ? ` else {\n${node.alternate.map(item => formatStatement(item, depth + 1)).join('\n')}\n${prefix}}` : ''}`;
