...
export const SELF_HOST_TRAIT_SOLVER_SOURCE = `pub fn trait_solver_version() -> String {
  return "1.2-kura-recursive-projection-specialization-solver"
}
...
fn solver_specificity(pattern: String) -> i32 {
  if pattern == "_" { return 0 }
  let mut score: i32 = 1
  let args = solver_type_args(pattern)
  let mut index: usize = 0
  while index < args.length {
    if args[index] != "_" { score += 2 }
    index += 1
  }
  return score
}

fn solver_impl_score(implementation) -> i32 {
  let mut score = implementation[9]
  if !implementation[8] { score += 100 }
  return score
}

fn solver_best_impl(implementations, trait_name: String, query_type: String, negative: bool) {
  let mut best: i32 = -1
  let mut best_score: i32 = -1
  let mut ambiguous = false
  let mut index: usize = 0
  while index < implementations.length {
    let implementation = implementations[index]
    if implementation[0] == trait_name && implementation[7] == negative && solver_patterns_overlap(implementation[1], query_type) {
      let score = solver_impl_score(implementation)
      if score > best_score { best = index; best_score = score; ambiguous = false }
...
fn solver_projection_parts(value: String) {
  let separator = value.indexOf("::")
  let open = value.indexOf("<")
  let close = value.lastIndexOf(">")
  if separator <= 0 || open <= separator || close <= open { return [] }
  return [value.slice(0, separator), value.slice(separator + 2, open), value.slice(open + 1, close)]
}

fn solver_normalize_projection(value: String, substitutions, implementations, depth: usize, trace) -> String {
  let substituted = solver_substitute(value, substitutions)
  if depth >= 8 { trace.push([substituted, substituted, "depth-limit"]); return substituted }
  let parts = solver_projection_parts(substituted)
  if parts.length == 0 { return substituted }
  let trait_name = parts[0]
  let assoc_name = parts[1]
  let projected_type = solver_normalize_projection(parts[2], substitutions, implementations, depth + 1, trace)
  let best = solver_best_impl(implementations, trait_name, projected_type, false)
  if best[0] < 0 || best[1] { trace.push([substituted, substituted, "unresolved"]); return substituted }
  let implementation = implementations[best[0]]
  let binding = solver_binding_value(implementation[2], assoc_name)
  if binding == "" { trace.push([substituted, substituted, "missing-binding"]); return substituted }
  let nested_substitutions = solver_match_substitutions(implementation[6], implementation[3], projected_type)
  let normalized = solver_normalize_projection(binding, nested_substitutions, implementations, depth + 1, trace)
  trace.push([substituted, normalized, "resolved"])
  return normalized
}
...
pub fn solve_trait_tokens(tokens_json: String, query_trait: String, query_type: String, query_assoc: String) -> String {
  ...
  while index < tokens.length {
    ...
    let is_default = solver_text(tokens, index - 1) == "default"
    ...
    if solver_text(tokens, cursor) == "!" { is_negative = true; cursor += 1 }
    ...
    implementations.push([trait_name, pattern, bindings, generics, tokens[index][2], tokens[index][3], raw_pattern, is_negative, is_default, solver_specificity(pattern)])
    index = body_close + 1
  }
  ...
  while impl_index < implementations.length {
    ...
    while other < implementations.length {
      let other_impl = implementations[other]
      if other_impl[0] == trait_name && solver_patterns_overlap(implementation[1], other_impl[1]) {
        if implementation[7] != other_impl[7] {
          diagnostics.push(["KR-SELF-TRAIT-1202", "Positive and negative implementations overlap for " + trait_name + " on " + implementation[1] + " and " + other_impl[1] + ".", other_impl[4], other_impl[5]])
        } else if implementation[7] {
          diagnostics.push(["KR-SELF-TRAIT-1203", "Overlapping negative implementations of " + trait_name + ".", other_impl[4], other_impl[5]])
        } else {
          let specialization_allowed = implementation[8] != other_impl[8] && implementation[9] != other_impl[9]
          if !specialization_allowed { diagnostics.push(["KR-SELF-TRAIT-1008", "Overlapping implementations of " + trait_name + " for " + implementation[1] + " and " + other_impl[1] + ".", other_impl[4], other_impl[5]]) }
        }
      }
      other += 1
    }
  ...
  let mut matches = []
  let mut projections = []
  let mut normalization_trace = []
  let mut specialization = []
  let mut negative_matches = []
  if query_trait != "" && query_type != "" {
    let negative_best = solver_best_impl(implementations, query_trait, query_type, true)
    if negative_best[0] >= 0 {
      negative_matches.push(implementations[negative_best[0]])
      diagnostics.push(["KR-SELF-TRAIT-1201", "Negative implementation rejects trait obligation " + query_trait + " for " + query_type + ".", 1, 1])
    } else {
      let positive_best = solver_best_impl(implementations, query_trait, query_type, false)
      if positive_best[0] < 0 { diagnostics.push(["KR-SELF-TRAIT-1009", "Unsatisfied trait obligation " + query_trait + " for " + query_type + ".", 1, 1]) }
      else if positive_best[1] { diagnostics.push(["KR-SELF-TRAIT-1010", "Ambiguous trait obligation " + query_trait + " for " + query_type + ".", 1, 1]) }
      else {
        matches.push(implementations[positive_best[0]])
        specialization.push([query_trait, query_type, implementations[positive_best[0]][1], positive_best[2], implementations[positive_best[0]][8]])
      }
    }
    if matches.length == 1 && requested_assoc != "" {
      ...
      if binding == "" { diagnostics.push(["KR-SELF-TRAIT-1102", "Associated type projection " + query_trait + "::" + requested_assoc + " is not defined for " + query_type + ".", 1, 1]) }
        ...
        let normalized = solver_normalize_projection(binding, substitutions, implementations, 0, normalization_trace)
        projections.push([requested_assoc, normalized, substitutions])
      }
    }
  }
  return JSON.stringify([diagnostics.length == 0, traits, implementations, diagnostics, matches, trait_solver_version(), projections, higher_ranked, negative_matches, specialization, normalization_trace])
}
`;

export const SELF_HOST_BORROW_CHECKER_SOURCE = `pub fn borrow_checker_version() -> String {
  return "2.1-kura-polonius-pin-generator-borrow-checker"
}
...
fn borrow_crosses_await(loans, point: usize, pinned) {
  let mut result = []
  let mut index: usize = 0
  while index < loans.length {
    let loan = loans[index]
    if loan[4] != "suspended" && loan[3] > point && !loan[1].startsWith("static.") && !loan[1].startsWith("global.") && !borrow_has_path(pinned, loan[1]) { result.push(loan) }
    index += 1
  }
  return result
}
...
pub fn check_borrow_paths(plan_json: String) -> String {
  ...
  while path_index < paths.length {
    ...
    while operation_index < operations.length {
      ...
      if kind == "pin" {
        if !borrow_has_path(pinned, target) { pinned.push(target) }
      }
      if kind == "await" {
        let crossings = borrow_crosses_await(loans, point, pinned)
        let mut crossing_index: usize = 0
        while crossing_index < crossings.length {
          diagnostics.push(["KR-SELF-BORROW-0304", "Reference " + crossings[crossing_index][0] + " to " + crossings[crossing_index][1] + " crosses await on path " + path_id + " without a pinned or stable async region.", line, column, path_id])
          crossing_index += 1
        }
        let mut loan_index: usize = 0
        while loan_index < loans.length {
          if loans[loan_index][3] > point { generator_fields.push([loans[loan_index][0], loans[loan_index][1], point, borrow_has_path(pinned, loans[loan_index][1])]) }
          loan_index += 1
        }
      }
      if kind == "move" {
    ...
    summaries.push([path_id, loans, moved, operations.length, pinned, generator_fields])
    path_index += 1
  }
...
export const SELF_HOST_CFG_REGION_SOURCE = `pub fn cfg_region_version() -> String {
  return "1.1-kura-polonius-loop-region-analyzer"
}
...
fn cfg_walk_paths(tokens, segments, start: usize, end: usize, id: String, output) {
  ...
  while index < end {
    ...
    let mut first_iteration = []
    cfg_walk_paths(tokens, before, open + 1, close, id + ".loop1", first_iteration)
    let mut first_index: usize = 0
    while first_index < first_iteration.length {
      loops.push(first_iteration[first_index])
      cfg_walk_paths(tokens, first_iteration[first_index][1], open + 1, close, id + ".loop2", loops)
      first_index += 1
    }
...
fn cfg_operations_for_path(tokens, path) {
  ...
  while position < indices.length {
    ...
    let text = cfg_text(tokens, index)
    if text == "pin" && cfg_text(tokens, index + 1) == "(" {
      let target_result = cfg_read_path(tokens, index + 2)
      if target_result[0] != "" {
        operations.push(["pin", "", target_result[0], "", point, point, tokens[index][2], tokens[index][3], ""])
...
fn cfg_fact_has(facts, kind: String, left: String, right: String, point: usize, path: String) -> bool {
  let mut index: usize = 0
  while index < facts.length {
    let fact = facts[index]
    if fact[0] == kind && fact[1] == left && fact[2] == right && fact[3] == point && fact[4] == path { return true }
    index += 1
  }
  return false
}

fn cfg_add_fact(facts, kind: String, left: String, right: String, point: usize, path: String) -> bool {
  if cfg_fact_has(facts, kind, left, right, point, path) { return false }
  facts.push([kind, left, right, point, path])
  return true
}

fn cfg_region_facts(plan) {
  let mut facts = []
  let mut path_index: usize = 0
  while path_index < plan.length {
    let path_id = plan[path_index][0]
    let operations = plan[path_index][1]
    let mut operation_index: usize = 0
    while operation_index < operations.length {
      let operation = operations[operation_index]
      let kind = operation[0]
      let point = operation[4]
      if operation_index + 1 < operations.length { cfg_add_fact(facts, "cfg_edge", "", "", operations[operation_index + 1][4], path_id) }
      if kind == "borrow" || kind == "reborrow" || kind == "reserve" || kind == "call_borrow" {
        cfg_add_fact(facts, "loan_issued", operation[1], operation[2], point, path_id)
        cfg_add_fact(facts, "loan_live", operation[1], operation[2], point, path_id)
        cfg_add_fact(facts, "loan_killed", operation[1], operation[2], operation[5] + 1, path_id)
      }
      if kind == "assign" || kind == "move" { cfg_add_fact(facts, "invalidates", "", operation[2], point, path_id) }
      if kind == "use_ref" { cfg_add_fact(facts, "requires", operation[1], operation[2], point, path_id) }
      if kind == "await" { cfg_add_fact(facts, "suspends", "", "", point, path_id) }
      if kind == "pin" { cfg_add_fact(facts, "pinned", "", operation[2], point, path_id) }
      operation_index += 1
    }
    path_index += 1
  }
  ...
  while changed && iterations < 64 {
    changed = false
    let snapshot_length = facts.length
    let mut fact_index: usize = 0
    while fact_index < snapshot_length {
      let fact = facts[fact_index]
      if fact[0] == "loan_live" {
        let mut edge_index: usize = 0
        while edge_index < facts.length {
          let edge = facts[edge_index]
          if edge[0] == "cfg_edge" && edge[4] == fact[4] && edge[3] > fact[3] {
            let mut killed = false
            let mut kill_index: usize = 0
            while kill_index < facts.length {
              let kill = facts[kill_index]
              if kill[0] == "loan_killed" && kill[1] == fact[1] && kill[4] == fact[4] && kill[3] <= edge[3] { killed = true }
              kill_index += 1
            }
            if !killed && cfg_add_fact(facts, "loan_live", fact[1], fact[2], edge[3], fact[4]) { changed = true }
          }
          edge_index += 1
        }
      }
  ...
  return [facts, iterations, !changed]
}

fn cfg_state_has(state, value: String) -> bool {
  let mut index: usize = 0
...
fn cfg_state_remove(state, prefix: String) {
  let mut result = []
  let mut index: usize = 0
  while index < state.length {
    if !state[index].startsWith(prefix) { result.push(state[index]) }
    index += 1
  }
  return result
}

fn cfg_transfer_loop_state(state, operations, diagnostics, loop_id: String) {
  let mut next = []
  let mut copy_index: usize = 0
  while copy_index < state.length { next.push(state[copy_index]); copy_index += 1 }
  let mut index: usize = 0
  while index < operations.length {
    let operation = operations[index]
    if operation[0] == "move" {
      let key = "m:" + operation[2]
      if cfg_state_has(next, key) { diagnostics.push(["KR-SELF-LOOP-0001", "Loop may move " + operation[2] + " again on a later iteration.", operation[6], operation[7]]) }
      else { next.push(key) }
    }
    if operation[0] == "assign" { next = cfg_state_remove(next, "m:" + operation[2]) }
    if operation[0] == "pin" && !cfg_state_has(next, "p:" + operation[2]) { next.push("p:" + operation[2]) }
    index += 1
  }
  return next
}

fn cfg_same_state(left, right) -> bool {
  if left.length != right.length { return false }
  let mut index: usize = 0
  while index < left.length {
    if !cfg_state_has(right, left[index]) { return false }
    index += 1
  }
  return true
}

fn cfg_loop_fixed_points(plan) {
  let mut results = []
  let mut seen = []
  let mut path_index: usize = 0
  while path_index < plan.length {
    let path_id = plan[path_index][0]
    let marker = path_id.indexOf(".loop1")
    if marker >= 0 {
      let loop_id = path_id.slice(0, marker)
      if !cfg_has(seen, loop_id) {
        seen.push(loop_id)
        let mut body_operations = plan[path_index][1]
        let mut state = []
        let mut diagnostics = []
        let mut changed = true
        let mut iterations: usize = 0
        while changed && iterations < 16 {
          let next = cfg_transfer_loop_state(state, body_operations, diagnostics, loop_id)
          changed = !cfg_same_state(state, next)
          state = next
          iterations += 1
        }
        results.push([loop_id, iterations, !changed, state, diagnostics])
      }
    }
    path_index += 1
  }
...
fn cfg_variance_facts(tokens) {
  let mut facts = []
  let mut index: usize = 0
  while index < tokens.length {
    if cfg_text(tokens, index) == "struct" {
      let name = cfg_text(tokens, index + 1)
      let mut cursor: usize = index + 2
      let mut generics = []
      if cfg_text(tokens, cursor) == "<" {
        let close = cfg_find_close(tokens, cursor, "<", ">")
        cursor += 1
        while cursor < close {
          if tokens[cursor][0] == "identifier" { cfg_add(generics, cfg_text(tokens, cursor)) }
          cursor += 1
        }
        cursor = close + 1
      }
      while cursor < tokens.length && cfg_text(tokens, cursor) != "{" { cursor += 1 }
      let body_close = cfg_find_close(tokens, cursor, "{", "}")
      let mut generic_index: usize = 0
      while generic_index < generics.length {
        let generic = generics[generic_index]
        let mut variance = "unused"
        let mut body_index: usize = cursor + 1
        while body_index < body_close {
          if cfg_text(tokens, body_index) == generic {
            if cfg_text(tokens, body_index - 1) == "mut" && cfg_text(tokens, body_index - 2) == "&" { variance = "invariant" }
            else if cfg_text(tokens, body_index - 1) == "&" && variance != "invariant" { variance = "covariant" }
            else if variance == "unused" { variance = "covariant" }
          }
          body_index += 1
        }
        facts.push([name, generic, variance])
        generic_index += 1
      }
      index = body_close + 1
      continue
...
fn cfg_lifetime_substitutions(contracts) {
  let mut substitutions = []
  let mut index: usize = 0
  while index < contracts.length {
    let contract = contracts[index]
    let mut variance = "covariant"
    if contract[2] == "mut" { variance = "invariant" }
    substitutions.push([contract[0], "output", contract[1], variance, contract[2]])
    index += 1
  }
  return substitutions
}

fn cfg_generator_layout(plan) {
  let mut layouts = []
  let mut path_index: usize = 0
  while path_index < plan.length {
    let path_id = plan[path_index][0]
    ...
    while operation_index < operations.length {
      let operation = operations[operation_index]
      if operation[0] == "pin" { cfg_add(pinned, operation[2]) }
      if operation[0] == "await" {
        let mut loan_index: usize = 0
        while loan_index < operations.length {
          let loan = operations[loan_index]
          if (loan[0] == "borrow" || loan[0] == "reborrow") && loan[4] < operation[4] && loan[5] > operation[4] {
            layouts.push([path_id, loan[1], loan[2], operation[4], cfg_has(pinned, loan[2]) || loan[2].startsWith("static.") || loan[2].startsWith("global.")])
          }
          loan_index += 1
        }
      }
...
pub fn build_cfg_regions(tokens_json: String) -> String {
  ...
  let region_facts = cfg_region_facts(plan)
  let loop_fixed_points = cfg_loop_fixed_points(plan)
  let mut loop_index: usize = 0
  while loop_index < loop_fixed_points.length {
    let loop_diagnostics = loop_fixed_points[loop_index][4]
    let mut diagnostic_index: usize = 0
    while diagnostic_index < loop_diagnostics.length { diagnostics.push(loop_diagnostics[diagnostic_index]); diagnostic_index += 1 }
    loop_index += 1
  }
  let variance_facts = cfg_variance_facts(tokens)
  let generator_layout = cfg_generator_layout(plan)
  let lifetime_substitutions = cfg_lifetime_substitutions(contracts)
  return JSON.stringify([diagnostics.length == 0, diagnostics, functions, all_paths, plan, contracts, captures, awaits, cfg_region_version(), region_facts, loop_fixed_points, variance_facts, generator_layout, lifetime_substitutions])
}
`;
...
export function createSelfHostMigrationManifest() {
  return Object.freeze({
    ...
    phase: 'polonius-loop-specialization-assisted-module-compiler',
    migrated: Object.freeze([
      ...
      'Kura-authored Polonius-style region fact generation and closure',
      'Kura-authored loop fixed-point move and pin state analysis',
      'Kura-authored generic variance classification',
      'Kura-authored pinned generator-field borrow modeling',
      'Kura-authored negative implementation and specialization selection',
      'Kura-authored recursive associated-type projection normalization',
    ...
    remaining: Object.freeze([
      ...
      'full chalk-equivalent trait canonicalization and specialization soundness proof',
      'irreducible control-flow, exception edges and complete Polonius equivalence',
...
function decodeTraitSolver(raw) {
  const [ok, traits, implementations, diagnostics, matches, version, projections = [], higherRanked = [], negativeMatches = [], specialization = [], normalizationTrace = []] = JSON.parse(raw);
  return { ok, traits, implementations, diagnostics: diagnostics.map(([code, message, line, column]) => ({ code, message, line, column })), matches, projections, higherRanked, negativeMatches, specialization, normalizationTrace, version };
}
function decodeCfgRegion(raw) {
  const [ok, diagnostics, functions, paths, plan, contracts, captures, awaits, version, regionFacts = [[], 0, true], loopFixedPoints = [], varianceFacts = [], generatorLayout = [], lifetimeSubstitutions = []] = JSON.parse(raw);
  return { ok, diagnostics: diagnostics.map(([code, message, line, column]) => ({ code, message, line, column })), functions, paths, plan, contracts, captures, awaits, version, regionFacts: { facts: regionFacts[0], iterations: regionFacts[1], converged: regionFacts[2] }, loopFixedPoints, varianceFacts, generatorLayout, lifetimeSubstitutions };
}
...
export async function bootstrapSelfHostedCompiler(options = {}) {
  ...
  const specializationProbeSource = 'trait Show { type Out }\nstruct Box<T> {}\ndefault impl<T> Show for Box<T> { type Out = T }\nimpl Show for Box<i32> { type Out = String }';
  const specializationProbe = decodeTraitSolver(traitSolverModule.solve_trait_tokens(frontendModule.tokenize_program(specializationProbeSource), 'Show', 'Box<i32>', 'Out'));
  if (!specializationProbe.ok || specializationProbe.projections[0]?.[1] !== 'String' || specializationProbe.specialization.length !== 1) throw new Error('Kura specialization ordering failed.');
  const negativeProbeSource = 'trait Send { type Marker }\nstruct Never {}\nimpl !Send for Never {}';
  const negativeProbe = decodeTraitSolver(traitSolverModule.solve_trait_tokens(frontendModule.tokenize_program(negativeProbeSource), 'Send', 'Never', ''));
  if (negativeProbe.ok || !negativeProbe.diagnostics.some(item => item.code === 'KR-SELF-TRAIT-1201')) throw new Error('Kura negative implementation checking failed.');
  const recursiveProjectionSource = 'trait Iterator { type Item }\ntrait Wrapper { type Output }\nstruct Box<T> {}\nimpl<T> Iterator for Box<T> { type Item = T }\nimpl<T> Wrapper for Box<T> { type Output = Iterator::Item<Box<T>> }';
  const recursiveProjectionProbe = decodeTraitSolver(traitSolverModule.solve_trait_tokens(frontendModule.tokenize_program(recursiveProjectionSource), 'Wrapper', 'Box<i32>', 'Output'));
  if (!recursiveProjectionProbe.ok || recursiveProjectionProbe.projections[0]?.[1] !== 'i32' || recursiveProjectionProbe.normalizationTrace.length === 0) throw new Error('Kura recursive associated-type projection failed.');
  ...
  if (lifetimeProbe.lifetimeSubstitutions[0]?.[2] !== 'value' || lifetimeProbe.lifetimeSubstitutions[0]?.[3] !== 'covariant') throw new Error('Kura lifetime substitution inference failed.');
  const poloniusSource = 'struct Shared<T> { value: &T }\nstruct Unique<T> { value: &mut T }\npub async fn pinned(mut value: String) -> String { pin(value)\nlet view = &value\nawait task()\nprint(view)\nreturn value }';
  const poloniusProbe = decodeCfgRegion(cfgRegionModule.build_cfg_regions(frontendModule.tokenize_program(poloniusSource)));
  const pinnedBorrowProbe = decodeBorrowChecker(borrowCheckerModule.check_borrow_paths(JSON.stringify(poloniusProbe.plan)), poloniusProbe.plan);
  if (!poloniusProbe.ok || !poloniusProbe.regionFacts.converged || poloniusProbe.regionFacts.facts.length === 0) throw new Error('Kura Polonius-style region fact closure failed.');
  if (poloniusProbe.varianceFacts[0]?.[2] !== 'covariant' || poloniusProbe.varianceFacts[1]?.[2] !== 'invariant') throw new Error('Kura variance inference failed.');
  if (!pinnedBorrowProbe.ok || poloniusProbe.generatorLayout[0]?.[4] !== true) throw new Error('Kura pinned generator borrow model failed.');
  const loopProbeSource = 'pub fn repeat(mut value: String, cond: bool) -> String { while cond { move value }\nreturn value }';
  const loopProbe = decodeCfgRegion(cfgRegionModule.build_cfg_regions(frontendModule.tokenize_program(loopProbeSource)));
  if (loopProbe.ok || !loopProbe.diagnostics.some(item => item.code === 'KR-SELF-LOOP-0001') || loopProbe.loopFixedPoints.length !== 1) throw new Error('Kura loop fixed-point dataflow failed.');
  ...
  return {
    ...
    frontendSelfAnalysis, frontendAst, frontendSymbols, frontendTypecheck, frontendSemanticTypecheck, expressionProbe, patternProbe, genericProbe, moveProbe, traitProbe, coherenceProbe, hrtbProbe, specializationProbe, negativeProbe, recursiveProjectionProbe, borrowProbe, borrowProbeCfg, borrowConflictProbe, borrowConflictCfg, reborrowProbe, reborrowCfg, twoPhaseProbe, twoPhaseCfg, closureProbe, closureCfg, asyncBorrowProbe, asyncBorrowCfg, lifetimeProbe, poloniusProbe, pinnedBorrowProbe, loopProbe, invalidAnalysis, invalidTypecheck, probeSource, probeCode, probeResult, probeAnalysis,
...
export async function writeSelfHostArtifacts(directory, options = {}) {
  ...
  await writeFile(files.report, JSON.stringify({ fixedPoint: result.fixedPoint, hashes: result.hashes, probeResult: result.probeResult, stage1Version: result.stage1Version, frontendVersion: result.frontendVersion, frontendSelfAnalysis: result.frontendSelfAnalysis, frontendAst: result.frontendAst, frontendSymbols: result.frontendSymbols, frontendTypecheck: result.frontendTypecheck, frontendSemanticTypecheck: result.frontendSemanticTypecheck, expressionProbe: result.expressionProbe, patternProbe: result.patternProbe, genericProbe: result.genericProbe, moveProbe: result.moveProbe, traitProbe: result.traitProbe, coherenceProbe: result.coherenceProbe, hrtbProbe: result.hrtbProbe, specializationProbe: result.specializationProbe, negativeProbe: result.negativeProbe, recursiveProjectionProbe: result.recursiveProjectionProbe, borrowProbe: result.borrowProbe, borrowProbeCfg: result.borrowProbeCfg, borrowConflictProbe: result.borrowConflictProbe, reborrowProbe: result.reborrowProbe, twoPhaseProbe: result.twoPhaseProbe, closureProbe: result.closureProbe, asyncBorrowProbe: result.asyncBorrowProbe, lifetimeProbe: result.lifetimeProbe, poloniusProbe: result.poloniusProbe, pinnedBorrowProbe: result.pinnedBorrowProbe, loopProbe: result.loopProbe, invalidAnalysis: result.invalidAnalysis, migration: result.migration, capabilities: result.capabilities }, null, 2) + '\n');
  return { ...result, files };
}
