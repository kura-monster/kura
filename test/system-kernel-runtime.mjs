...
assert.match(source, /pml4_entry = pml4_entry \| PAGE_USER/);
assert.match(source, /pdpt_entry = pdpt_entry \| PAGE_USER/);
assert.match(source, /directory_entry = directory_entry \| PAGE_USER/);
...
assert.match(source, /const HEAP_HEADER_SIZE: usize = 32/);
assert.match(source, /const HEAP_MIN_BLOCK_SIZE: usize = 64/);
assert.match(source, /static mut HEAP_FREE_HEAD: usize = 0/);
assert.match(source, /fn heap_alignment_is_valid\(alignment: usize\) -> bool/);
assert.match(source, /unsafe fn heap_block_is_valid\(block: usize\) -> bool/);
assert.match(source, /unsafe fn heap_free_insert\(block: usize\)/);
assert.match(source, /unsafe fn heap_free_remove\(block: usize\)/);
assert.match(source, /unsafe fn heap_address_to_block\(address: usize\) -> usize/);
assert.match(source, /pub unsafe fn heap_alloc\(bytes: usize, alignment: usize\) -> usize/);
assert.match(source, /pub unsafe fn heap_alloc_zeroed\(bytes: usize, alignment: usize\) -> usize/);
assert.match(source, /pub unsafe fn heap_usable_size\(address: usize\) -> usize/);
assert.match(source, /pub unsafe fn heap_free\(address: usize\) -> bool/);
assert.match(source, /pub unsafe fn heap_realloc\(address: usize, bytes: usize, alignment: usize\) -> usize/);
assert.match(source, /pub unsafe fn heap_total_bytes\(\) -> usize/);
assert.match(source, /pub unsafe fn heap_allocated_bytes\(\) -> usize/);
assert.match(source, /pub unsafe fn heap_free_bytes\(\) -> usize/);
assert.match(source, /pub unsafe fn heap_allocation_count\(\) -> usize/);
assert.match(source, /pub unsafe fn heap_largest_free_block\(\) -> usize/);
assert.match(source, /pub unsafe fn heap_validate\(\) -> bool/);
assert.match(source, /pub unsafe fn heap_runtime_self_test\(\) -> bool/);
assert.match(source, /let reused: usize = heap_alloc\(96, 64\)/);
assert.match(source, /let grown: usize = heap_realloc\(first, 256, 16\)/);
assert.match(source, /heap_free_bytes\(\) == initial_free/);
assert.match(source, /heap_init\(SAFE_HEAP_BASE, SAFE_HEAP_SIZE\)/);
assert.match(source, /let heap_runtime_ready: bool = heap_runtime_self_test\(\)/);
assert.match(source, /serial_write_byte\(0x48\)/);
assert.match(source, /io\.out32\(0xF4, 18\)/);
...
assert.match(llvm, /define .*@heap_block_is_valid\(/);
assert.match(llvm, /define .*@heap_alloc\(/);
assert.match(llvm, /define .*@heap_alloc_zeroed\(/);
assert.match(llvm, /define .*@heap_free\(/);
assert.match(llvm, /define .*@heap_realloc\(/);
assert.match(llvm, /define .*@heap_validate\(/);
assert.match(llvm, /define .*@heap_runtime_self_test\(/);
assert.match(llvm, /load i8, ptr/);
assert.match(llvm, /store i8/);
assert.match(llvm, /load i32, ptr/);
assert.match(llvm, /load i64, ptr/);
assert.match(llvm, /store i64/);
